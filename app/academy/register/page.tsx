...
import { UserPlus, Mail, Lock, Phone, User, Globe, Loader2 } from "lucide-react";
...
const StudentRegister = () => {
  ...
  useEffect(() => {
    ...
    if (savedCommunity) {
      setFormData(prev => ({ ...prev, community: savedCommunity }));
    } else {
      // If no community selected, redirect back to academy to choose one
      router.push("/academy");
    }
  }, [router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, "users", user.uid), {
        ...
        role: formData.role,
        isApproved: true, // Students are approved by default
    ...
    <div className="min-h-screen pt-32 pb-20 bg-african-pattern flex items-center justify-center px-4">
      ...
      >
        <div className="text-center mb-10">
          ...
          <p className="text-foreground/60">Join our community and start your tech journey today.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-foreground/70 ml-1">Joined Community</label>
            <div className="relative">
              <Globe className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-foreground/40" />
              <input
                readOnly
                type="text"
                className="w-full bg-white/5 border border-white/10 rounded-xl py-4 pl-12 pr-4 focus:outline-none cursor-not-allowed capitalize"
                value={formData.community}
              />
            </div>
            <p className="text-[10px] text-foreground/40 ml-1">Community is selected based on your previous choice.</p>
          </div>
